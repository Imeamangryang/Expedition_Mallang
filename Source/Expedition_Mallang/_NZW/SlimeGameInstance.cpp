...
bool USlimeGameInstance::GetStatData(int32 Level, FPlayerStat& OutStat) const
{
	if (!StatData)
	{
		UE_LOG(LogTemp, Error, TEXT("StatDataTable 할당안됨"));
		return false;
	}
	...
	if (!Row)
	{
		UE_LOG(LogTemp, Error, TEXT("%s 행을 찾을 수 없음"), *RowName);
		return false;
	}
...
void USlimeGameInstance::PlayGameFirst()
{
	PlaySaveGame->bIsPlayFirst = true;
}
