...
ASlimeVacpack::ASlimeVacpack()
{
	...
	Muzzle->SetRelativeLocationAndRotation(FVector(0.0f, 63.0f, 11.0f), FRotator(0.0f, 90.0f, 0.0f));
...
void ASlimeVacpack::Tick(float DeltaTime)
{
	...
	{
		// RMB 누르는 동안 매 프레임:
		// 1) 감지 범위를 다시 스캔해서 목록 갱신 (벗어난 슬라임 자동 제거됨)
		VacuumDetecting();
		
		// 2) 감지된 슬라임이 있으면 흡입 물리 적용
...
void ASlimeVacpack::Vacuuming(float DeltaTime)
{
	...
	for (AActor* VacuumableActor : CurrentVacuumTargets)
	{
		...
		//*
		//* [버그 수정] 기존 코드는 VacuumableVelocities 하나에 FVector를 저장했는데
		...
		//// SpringK = 4.0f;	// 스프링 세기: 클수록 중심으로 더 빠르게 수렴
		//// DamperC = 2.0f;	// 댐퍼 세기: 클수록 진동이 빨리 사라짐
		...
		//// float VacForce    = 3000.0f;	// 흡입 세기 (클수록 빠르게 당김)
		...
		//// float SwirlStrength = 150.0f;	// 소용돌이 세기 (클수록 더 빠르게 회전)
